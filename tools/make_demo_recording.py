...
# The real DupleX geometry, so the array in the window looks like the array
# on the bench. At the instrument's own 18.5 kHz that is 152 MB/s, so the
# demo runs at a slower frame rate instead - a 4096-electrode picture matters
# for learning the controls; the sample rate does not, and the sidecar says
# what this is.
FRAME_RATE_HZ = 1000.0
N_CHANNELS = 4096        # 64 x 64, as on the instrument
SECONDS = 8.0
...
def main(argv=None) -> int:
    ...
    # Per-electrode noise amplitude, so the array has visible structure
    # rather than being uniform: a lively patch, a quiet corner, and a few
    # dead electrodes. The window colours by peak-to-peak, so it is the
    # variance that shows, not the offset.
    rows, cols = np.mgrid[0:side, 0:side]
    centre = np.exp(-(((rows - 20) ** 2 + (cols - 26) ** 2) / 90.0))
    second = np.exp(-(((rows - 45) ** 2 + (cols - 40) ** 2) / 40.0))
    gain = (8.0 + 110.0 * centre + 70.0 * second).ravel()
